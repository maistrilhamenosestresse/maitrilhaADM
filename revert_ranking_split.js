...
const parts = content.split('/* Ranking dos Top Clientes */');
if (parts.length !== 2) {
  console.error("Could not find exactly one instance of the ranking comment.");
  process.exit(1);
}
...
if (endIdx === -1) {
  console.error("Could not find the end of the ranking block.");
  process.exit(1);
}

const afterBlock = after.substring(endIdx + 5);
...
fs.writeFileSync(path, before + newBlock + afterBlock, 'utf8');
console.log("Successfully replaced block using split.");
